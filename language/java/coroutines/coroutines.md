...
Просто так вызвать suspend функцию не получится. Ее можно вызвать только их другой suspend функции. Чтобы решить данную проблему есть **CoroutineBuilder**.
...
- Job.cancel() - отменит только одну корутину. Она опевестит об этом своего родителя. Остальные корутины продолжат выполнение.
